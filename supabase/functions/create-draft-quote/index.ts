...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // IDEMPOTENCY STEP C: Validate status - only allow draft creation from valid states
    ...
    // CRITICAL: Apply user corrections BEFORE creating quote
    if (userCorrections) {
      ...
      extracted = JSON.parse(JSON.stringify(extracted)); // Deep clone
    ...
    // CRITICAL: If user has confirmed, skip all quality guards
    if (userConfirmed) {
      ...
      // VOICE QUALITY GUARDS: Block quote creation if data quality is insufficient
      console.log("[QUOTE_CREATE] Checking voice quality guards (user NOT confirmed)", { intake_id });

      // Guard 1: Block if any required fields are missing
      ...
      // Guard 2: Block if labour hours confidence is too low
      ...
      // Guard 3: Check if user review is required
    ...
    // IDEMPOTENCY STEP D: Create quote exactly once (only if created_quote_id is NULL)
    // At this point we know:
    // - The row is locked
    // - created_quote_id is NULL
    // - status is valid (extracted)
    // - User review is NOT required

    // CRITICAL: Get pricing profile at runtime - NEVER use cached or client-supplied pricing
    const { data: profileData, error: profileError } = await supabase
      .rpc("get_effective_pricing_profile", { p_user_id: user.id });

    // DEFENSIVE LOG: Always log pricing lookup result
    ...
    // Create pricing snapshot for audit trail
    ...
    // Create quote with profile defaults
    ...
    // LABOUR ITEMS - Use hourly_rate_cents from profile
    if (extracted.time?.labour_entries) {
      for (const labour of extracted.time.labour_entries) {
        // Handle both old format (direct value) and new format (value + confidence)
        ...
        // Convert days to hours using workday_hours_default
        if (!hours && days) {
        ...
        if (hours) {
          ...
          const lineTotalCents = Math.round(totalHours * profile.hourly_rate_cents);
    ...
    // MATERIALS ITEMS - Apply materials_markup_percent
    if (extracted.materials?.items) {
      for (const material of extracted.materials.items) {
        // Handle both old format and new format with confidence
        const quantity = typeof material.quantity === "object" ? material.quantity?.value : material.quantity;
        ...
        if (material.unit_price_cents && material.unit_price_cents > 0) {
          ...
          // Apply markup to estimated cost (for bulk items)
    ...
    if (extracted.fees?.travel) {
      ...
      if (travel.is_time !== undefined ? travel.is_time : profile.travel_is_time) {
        ...
        // Use explicitly provided travel fee
        lineItems.push({
          org_id: profile.org_id,
          quote_id: quote.id,
          item_type: "fee",
    ...
    // BUNNINGS RUN - Use bunnings_run_enabled and default minutes
    ...
    // Store pricing snapshot in voice_intakes
    const updatedExtractionJson = {
      ...extracted,
      pricing_used: pricingSnapshot,
    };

    // Quote created successfully - update intake to quote_created status
    await supabase
      .from("voice_intakes")
      .update({
