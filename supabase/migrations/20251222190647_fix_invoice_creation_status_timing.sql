/*
  # Fix Invoice Creation - Status Timing Bug
  
  1. Problem
    - Function creates invoice with status='issued'
    - Then tries to add line items
    - Trigger blocks line item insertion for 'issued' invoices
    - This creates circular logic failure
  
  2. Root Cause
    - Invoice status set to 'issued' too early
    - Trigger prevents modifications to issued invoices
    - But line items need to be added during creation
  
  3. Solution
    - Create invoice with status='draft'
    - Add all line items
    - Recalculate totals
    - THEN update status to 'issued'
  
  4. Changes
    - Change initial status from 'issued' to 'draft'
    - Add UPDATE at end to set status='issued' after line items added
*/
...
COMMENT ON FUNCTION public.create_invoice_from_accepted_quote(uuid) IS 'Creates an invoice from an accepted quote. Creates as draft, adds line items, then marks as issued.';
