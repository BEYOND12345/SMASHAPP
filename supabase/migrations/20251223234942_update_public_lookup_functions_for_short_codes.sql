/*
  # Update Public Lookup Functions for Short Codes

  1. Changes
    - Update `get_public_quote()` to accept either UUID or short_code
    - Create `get_public_invoice()` to accept either UUID or short_code
    - Maintain backward compatibility with existing UUID-based lookups
  
  2. Logic
    - Try to parse input as UUID first
    - If UUID parsing fails, treat as short_code
    - Return same data structure as before
  
  3. Security
    - No changes to RLS or permissions
    - Functions remain SECURITY DEFINER with public access
    - Only return data meant for public viewing
*/

-- Update get_public_quote to support both UUID and short_code
...
-- Create get_public_invoice function with short_code support
